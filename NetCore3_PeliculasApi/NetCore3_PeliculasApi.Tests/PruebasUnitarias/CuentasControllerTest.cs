...
namespace NetCore3_PeliculasApi.Tests.PruebasUnitarias
{
    ...
    public class CuentasControllerTests:BasePruebas
    {
        ...
        public async Task UsuarioPuedeLoguearse()
        {
            ...
            var controller = ConstruirCuentasController(nombreBD);
            var userInfo = new UserInfo() { Email = "ejemplo@hotmail.com", Password = "ASDAaSD12323!" };
            var respuesta = await controller.Login(userInfo);
        ...
        private CuentasController ConstruirCuentasController(string nombreBD)
        {
            ...
            MockAut(httpContext);
            var singInManager = SetupSignInManager(userManager, httpContext);

            var miConfiguracion = new Dictionary<string, string>
            {
                {"JWT:key","sfsdfsSDFSDFSDFSDFSDFSDF54645W4ERWE4564SD5FSSD2FSD32F1DS" }
            };
            ...
            return new CuentasController(userManager, singInManager, configuration, context, mapper);
        }
        

        // source: https://github.com/dotnet/aspnetcore/blob/master/src/Identity/test/Shared/MockHelpers.cs
        // https://github.com/dotnet/aspnetcore/blob/master/src/Identity/test/Identity.Test/SignInManagerTest.cs
        ...
        private Mock<IAuthenticationService> MockAut(HttpContext context)
        {
